...
@EndToEndTest
public class EdcCompatibilityDspV08PostgresTest {
    ...
    private static final RuntimeExtension RUNTIME = new RuntimePerClassExtension(new EmbeddedRuntime(CONNECTOR_UNDER_TEST,
            ...
            .configurationProvider(() -> EdcCompatibilityDspV08PostgresTest.runtimeConfiguration().merge(POSTGRES.getConfig(CONNECTOR_UNDER_TEST))));

    private static final GenericContainer<?> TCK_CONTAINER_08 = new TckContainer<>("eclipsedataspacetck/dsp-tck-runtime:1.0.0-RC4");
    ...
    @Timeout(500)
    @Test
    void assertDspCompatibilityWithProtocolUrl08() throws Exception {
        var monitor = new ConsoleMonitor(">>> TCK Runtime (Docker)", ConsoleMonitor.Level.INFO, true);
        var reporter = new TckTestReporter();

        // Load and override properties
        var properties = new java.util.Properties();
        try (var reader = new java.io.FileReader(resourceConfig("docker.tck.dsp.v08.properties"))) {
            properties.load(reader);
        }
        properties.put("dataspacetck.dsp.connector.http.url", PROTOCOL_URL.toString());
        // Write to a temp file
        var tempFile = java.io.File.createTempFile("tck-protocol-url-08", ".properties");
        try (var writer = new java.io.FileWriter(tempFile)) {
            properties.store(writer, null);
        }

        TCK_CONTAINER_08.addFileSystemBind(tempFile.getAbsolutePath(),
                "/etc/tck/config.properties", BindMode.READ_ONLY, SelinuxContext.SINGLE);
        TCK_CONTAINER_08.withCopyFileToContainer(MountableFile.forClasspathResource("document/dspace-edc-context-v1.jsonld"),
                "/etc/tck/dspace-edc-context-v1.jsonld");
        TCK_CONTAINER_08.withCopyFileToContainer(MountableFile.forClasspathResource("document/tx-auth-v1.jsonld"),
                "/etc/tck/tx-auth-v1.jsonld");
        TCK_CONTAINER_08.withCopyFileToContainer(MountableFile.forClasspathResource("document/cx-policy-v1.jsonld"),
                "/etc/tck/cx-policy-v1.jsonld");
        TCK_CONTAINER_08.withExtraHost("host.docker.internal",
                "host-gateway");
        TCK_CONTAINER_08.withLogConsumer(outputFrame -> monitor.info(outputFrame.getUtf8String()));
        TCK_CONTAINER_08.withLogConsumer(reporter);
        TCK_CONTAINER_08.waitingFor(new LogMessageWaitStrategy().withRegEx(".*Test run complete.*").withStartupTimeout(Duration.ofSeconds(300)));
        TCK_CONTAINER_08.start();
    ...
    @Timeout(500)
    @Test
    void assertDspCompatibilityWithProtocolUrl082() {
        var monitor = new ConsoleMonitor(">>> TCK Runtime (Docker)", ConsoleMonitor.Level.INFO, true);
        var reporter = new TckTestReporter();

        TCK_CONTAINER_08.addFileSystemBind(resourceConfig("docker.tck.dsp.v08.properties"),
                "/etc/tck/config.properties", BindMode.READ_ONLY, SelinuxContext.SINGLE);
        TCK_CONTAINER_08.withCopyFileToContainer(MountableFile.forClasspathResource("document/dspace-edc-context-v1.jsonld"),
                "/etc/tck/dspace-edc-context-v1.jsonld");
        TCK_CONTAINER_08.withCopyFileToContainer(MountableFile.forClasspathResource("document/tx-auth-v1.jsonld"),
