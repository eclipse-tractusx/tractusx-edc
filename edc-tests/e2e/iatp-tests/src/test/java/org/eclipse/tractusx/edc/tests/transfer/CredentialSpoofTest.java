...
public class CredentialSpoofTest {
    ...
    /*@RegisterExtension
    protected static WireMockExtension csServiceServer = WireMockExtension.newInstance()
            .options(wireMockConfig().bindAddress("localhost").port(MOCKED_CS_SERVICE_PORT))
            .build();*/
    ...
    void setup() {
        ...
        server.start();
        csServiceServer = new WireMockServer(options().bindAddress("localhost").port(MOCKED_CS_SERVICE_PORT));
        csServiceServer.start();*/
    ...
    void withMock(Function<VerifiableCredentialResource, Result<PresentationResponseMessage>> response) {
        ...
        /*csServiceServer.stubFor(post(urlPathEqualTo("/presentations/query"))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody(json.toString())));*/
