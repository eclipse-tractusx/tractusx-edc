...
import org.eclipse.tractusx.edc.tests.transfer.dcp.harness.StsParticipant;
...
public class CredentialSpoofTest {
    ...
    private static final DcpParticipant CONSUMER = participant(CONSUMER_NAME, CONSUMER_BPN);
    private static final DcpParticipant PROVIDER = participant(PROVIDER_NAME, PROVIDER_BPN);
    private static final DcpParticipant MALICIOUS_ACTOR = participant("MALICIOUS", "BPNL000MALICIOUS");

    @RegisterExtension
    protected static final RuntimeExtension MALICIOUS_ACTOR_RUNTIME = dcpRuntime(MALICIOUS_ACTOR.getName(), MALICIOUS_ACTOR.getKeyPair(),
            () -> MALICIOUS_ACTOR.dcpConfig().merge(BDRS_SERVER_EXTENSION.getConfig()));
    @RegisterExtension
    protected static final RuntimeExtension CONSUMER_RUNTIME = dcpRuntime(CONSUMER.getName(), CONSUMER.getKeyPair(),
            () -> CONSUMER.dcpConfig().merge(BDRS_SERVER_EXTENSION.getConfig()));
    @RegisterExtension
    protected static final RuntimeExtension PROVIDER_RUNTIME = dcpRuntime(PROVIDER.getName(), PROVIDER.getKeyPair(),
            () -> PROVIDER.dcpConfig().merge(BDRS_SERVER_EXTENSION.getConfig()));
    @RegisterExtension
    ...
    private static DcpParticipant participant(String name, String bpn) {
        return DcpParticipant.Builder.newInstance().name(name).id(bpn)
                .stsUri(STS.stsUri())
                .bpn(bpn)
                .stsClientId(bpn)
