...
package org.eclipse.tractusx.edc.tests.transfer.dcp.runtime;
...
public interface Runtimes {
    ...
    static RuntimeExtension divRuntime(String name, KeyPair keyPair, Supplier<Config> configurationProvider) {
        return genericRuntime(name, ":edc-tests:runtime:dcp:runtime-memory-dcp-div-ih", keyPair, configurationProvider)
                .registerSystemExtension(ServiceExtension.class, new VaultSeedExtension(Map.of("client_secret_alias", "client_secret")));
    }

    static RuntimeExtension dcpRuntime(String name, KeyPair keyPair, Supplier<Config> configurationProvider) {
        return genericRuntime(name, ":edc-tests:runtime:dcp:runtime-memory-dcp-ih", keyPair, configurationProvider)
                .registerSystemExtension(ServiceExtension.class, new VaultSeedExtension(Map.of("client_secret_alias", "client_secret")));
    }

    static RuntimeExtension stsRuntime(String name, KeyPair keyPair, Supplier<Config> configurationProvider) {
        return new RuntimePerClassExtension(new EmbeddedRuntime(name, ":edc-tests:runtime:dcp:runtime-memory-sts").configurationProvider(configurationProvider)
                .registerSystemExtension(ServiceExtension.class, new VaultSeedExtension(Map.of("client_secret_alias", "client_secret"))))
    ...
    private static RuntimeExtension genericRuntime(String name, String moduleName, KeyPair keyPair, Supplier<Config> configurationProvider) {
        return new DcpParticipantRuntimeExtension(
