 ...
 *
 * Did Document is tied to a company identity in DIV, which is resolved using the own DID. Company identity is needed
 * to perform any updates to the DID Document.
 *
 * <p>
 * Did document update is a two-step process in DIV:
 ...
 *     PATCH {divUrl}/companyIdentities/{companyIdentityId}/status
 */
public class DidDocumentServiceDivClient implements DidDocumentServiceClient {
    ...
    private final DivOauth2Client divOauth2Client;
    ...
    public DidDocumentServiceDivClient(EdcHttpClient httpClient,
                                       DivOauth2Client divOauth2Client, ObjectMapper mapper, String divHost, String ownDid, Monitor monitor) {
        this.httpClient = httpClient;
        this.divOauth2Client = divOauth2Client;
        this.mapper = mapper;
        this.ownDid = ownDid;
        this.monitor = monitor.withPrefix(getClass().getSimpleName());
        this.didDocApiUrl = String.join("", divHost, DID_DOC_API_PATH);
    }
    ...
    Result<String> handleDidUpdateResponse(Response response) {
        ...
        try {
            ...
            monitor.severe("Failed to parse did update response from DIV", e);
    ...
    Result<String> handlePatchStatusResponse(Response response) {
        ...
        try {
            ...
            monitor.severe("Failed to parse patch status response from DIV", e);
            return Result.failure(e.getMessage());
        }
    }
     ...
     * Package Private visibility for testing.
     * <p>
     * It is expected that DIV returns exactly one company identity for the given DID.
    ...
    Result<String> handleCompanyIdentityResponse(Response response) {
        ...
        try {
            ...
            monitor.severe("Failed to parse company identity response from DIV", e);
            return Result.failure(e.getMessage());
        }
    }
    ...
    private Result<Request.Builder> baseRequestWithToken() {
        return divOauth2Client.obtainRequestToken().map(this::baseRequestWithToken);
    }
