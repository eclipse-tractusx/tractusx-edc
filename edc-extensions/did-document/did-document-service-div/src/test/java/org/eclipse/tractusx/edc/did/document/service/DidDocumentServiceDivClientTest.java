...
class DidDocumentServiceDivClientTest {
    ...
    private final String divHost = "https://div.example.com";
    private final String didDocApiUrl = String.join("", divHost, "/api/v2.0.0/companyIdentities");
    ...
    void setUp() {
        ...
        client = new DidDocumentServiceDivClient(httpClient, divOauth2Client, mapper, divHost, ownDid, monitor);
    }

    @Test
    void update_service_success() {

        when(divOauth2Client.obtainRequestToken()).thenReturn(Result.success(TokenRepresentation.Builder.newInstance().token("token").build()));
