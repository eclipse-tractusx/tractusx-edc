 ...
 */
public class JurisdictionLocationConstraintFunction<C extends ParticipantAgentPolicyContext> implements AtomicConstraintRuleFunction<Permission, C> {
    public static final String JURISDICTION_LOCATION_LITTERAL = "JurisdictionLocation";
    ...
    public Result<Void> validate(Operator operator, Object rightValue, Permission rule) {
        ...
        return rightValue instanceof String
                ? Result.success()
                : Result.failure("Invalid right-operand: this constraint only allows string values");
    }
}