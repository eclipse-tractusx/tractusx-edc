...
class ConfidentialInformationMeasuresConstraintFunctionTests {

    private final ParticipantAgent participantAgent = mock();
    private final ConfidentialInformationMeasuresConstraintFunction<ParticipantAgentPolicyContext> function = new ConfidentialInformationMeasuresConstraintFunction<>();
    private final ParticipantAgentPolicyContext context = new TestParticipantAgentPolicyContext(participantAgent);

    @Test
    void evaluate() {
        assertThat(function.evaluate(Operator.EQ, "cx.confidentiality.measures:1", null, context)).isTrue();
    }

    @Test
    void validate_whenOperatorAndRightOperandAreValid_thenSuccess() {
        var result = function.validate(Operator.EQ, "cx.confidentiality.measures:1", null);
        assertThat(result.succeeded()).isTrue();
    }

    @Test
    void validate_whenInvalidOperator_thenFailure() {
        var result = function.validate(Operator.IS_ANY_OF, "cx.confidentiality.measures:1", null);
