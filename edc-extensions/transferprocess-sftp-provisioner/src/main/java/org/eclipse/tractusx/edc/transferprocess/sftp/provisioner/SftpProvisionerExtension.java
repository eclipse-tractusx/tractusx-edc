...
import org.eclipse.tractusx.edc.trasnferprocess.sftp.common.SftpSettings;
...
public class SftpProvisionerExtension implements ServiceExtension, SftpSettings {
  ...
  public void initialize(ServiceExtensionContext context) {
    ...
    SftpLocationFactory locationFactory =
        new ConfigBackedSftpLocationFactory(sftpHost, sftpPort, sftpPath);
    ...
    SftpUserFactory userFactory =
        ConfigBackedSftpUserFactory.builder()
            .sftpUserName(sftpName)
            .sftpUserKeyPair(sftpKeyPair)
            .sftpUserPassword(sftpPassword)
            .build();
    ...
    for (var configuration : configurations) {
      sftpProvisioner =
          new SftpProvisioner(policyEngine, locationFactory, userFactory, sftpProvider);

      if (configuration.getProvisionerType()
          == SftpProvisionerConfiguration.ProvisionerType.PROVIDER) {
        var generator =
            new SftpProviderResourceDefinitionGenerator(configuration.getDataAddressType());
        manifestGenerator.registerGenerator(generator);
        monitor.info(
            String.format(
                "Registering provider provisioner: %s [%s]",
                configuration.getName(), configuration.getEndpoint()));
      } else {
        monitor.warning(
            String.format(
                "Client-side provisioning not yet supported by the %s. Skipping configuration for %s",
