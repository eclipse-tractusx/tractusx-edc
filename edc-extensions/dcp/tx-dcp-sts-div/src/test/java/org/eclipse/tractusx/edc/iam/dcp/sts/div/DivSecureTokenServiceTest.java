...
public class DivSecureTokenServiceTest {
    ...
    private DivSecureTokenService client;

    @BeforeEach
    void setup() {
        when(monitor.withPrefix(anyString())).thenReturn(monitor);
        client = new DivSecureTokenService(testHttpClient(interceptor), DIV_URL, oauth2Client, mapper, monitor);
    }
    ...
    void createToken_grantAccess() throws IOException {
        ...
        Consumer<Request> requestAcceptor = (request) -> {
            ...
            assertThat(request.url().url().toString()).isEqualTo(DIV_URL);
    ...
    @Test
    void createToken_grantFails_withDivFailure() throws IOException {
    ...
    @Test
    void createToken_signFails_withDivFailure() throws IOException {
