...
public class DcpDefaultScopeExtensionTest {
    ...
    @Test
    void initialize_withConfiguredScopes(ServiceExtensionContext context, DcpDefaultScopeExtension extension) {
    ...
    @Test
    void initialize_fails_withBadConfiguredScopes(ServiceExtensionContext context, DcpDefaultScopeExtension extension) {
        var cfg = ConfigFactory.fromMap(Map.of(
                "foo.alias", "org.test.alias.foo"
        ));
        when(context.getConfig(TX_DCP_DEFAULT_SCOPE_PREFIX)).thenReturn(cfg);
