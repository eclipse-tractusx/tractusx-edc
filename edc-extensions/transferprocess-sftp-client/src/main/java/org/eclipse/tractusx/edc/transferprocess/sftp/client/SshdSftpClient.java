...
public class SshdSftpClient implements SftpClientWrapper {
    ...
    public void uploadFile(@NonNull final SftpUser sftpUser, @NonNull final SftpLocation sftpLocation, @NonNull final InputStream inputStream) throws IOException {
        try (final SftpClient sftpClient = getSftpClient(sftpUser, sftpLocation)) {
            try (OutputStream outputStream = sftpClient.write(sftpLocation.getPath(), BUFFER_SIZE_DEFAULT)) {
    ...
    private SftpClient getSftpClient(SftpUser sftpUser, SftpLocation sftpLocation) {
        ...
        if (sftpUser.getKeyPair() != null) {
            ...
            sshClient.setPasswordIdentityProvider(PasswordIdentityProvider.wrapPasswords(sftpUser.getPassword()));
        } else {
            throw new EdcException(String.format("No authentication method provided for sftp user %s", sftpUser.getName()));
        }
        sshClient.start();

        ClientSession session = sshClient.connect(sftpUser.getName(), sftpLocation.getHost(), sftpLocation.getPort()).verify().getSession();
