...
public class JsonObjectFromPolicyTransformer extends AbstractJsonLdTransformer<Policy, JsonObject> {
    ...
    private static class Visitor implements Policy.Visitor<JsonObject>, Rule.Visitor<JsonObject>, Constraint.Visitor<JsonObject>, Expression.Visitor<JsonObject> {
        ...
        public JsonObject visitAtomicConstraint(AtomicConstraint atomicConstraint) {
            ...
            constraintBuilder.add(ODRL_RIGHT_OPERAND_ATTRIBUTE, rightOperand(atomicConstraint));

            return constraintBuilder.build();
        }

        /**
         * Renders the right operand of an atomic constraint. A set-based operator such as {@code isAnyOf} carries a
         * list of values, which {@link #visitLiteralExpression} would render as the list's {@code toString()}; emit a
         * JSON-LD array of literals instead so the values survive the round trip.
         */
        private JsonValue rightOperand(AtomicConstraint atomicConstraint) {
            if (atomicConstraint.getRightExpression() instanceof LiteralExpression literal &&
                    literal.getValue() instanceof Collection<?> values) {
                return values.stream()
                        .map(value -> jsonFactory.createObjectBuilder().add(VALUE, Json.createValue(scalar(value))).build())
                        .collect(toJsonArray());
            }
            return atomicConstraint.getRightExpression().accept(this);
        }

        /**
         * Unwraps a single value of a multi-valued right operand. Such values are not expanded when the policy is
         * transformed, so they may still be JSON-LD literals, or the map form those deserialize to.
         */
        private String scalar(Object value) {
            if (value instanceof JsonString jsonString) {
                return jsonString.getString();
            }
            if (value instanceof JsonObject jsonObject && jsonObject.get(VALUE) != null) {
                return scalar(jsonObject.get(VALUE));
            }
            if (value instanceof Map<?, ?> map) {
                var nested = map.containsKey(VALUE) ? map.get(VALUE) : map.get("string");
                return nested == null ? value.toString() : scalar(nested);
            }
