...
package org.eclipse.tractusx.edc.postgresql.migration.dataplane;
...
import static org.eclipse.tractusx.edc.postgresql.migration.dataplane.DataPlanePostgresqlMigration.NAME;
import static org.flywaydb.core.api.MigrationVersion.LATEST;

@Extension(NAME)
public class DataPlanePostgresqlMigration implements ServiceExtension {

    public static final String NAME = "Data-plane Postgresql Schema Migration";
    ...
    public void prepare() {
        if (!enabled) {
            monitor.info("Migration for data plane disabled");
            return;
        }
        ...
        var flyway = Flyway.configure()
                ...
                .locations("classpath:migrations/data-plane")
        ...
        if (!migrateResult.success) {
            throw new EdcPersistenceException(
                    "Migrating data-plane failed: %s".formatted(String.join(", ", migrateResult.warnings))
            );
        }
    }
