...
- The existing access policies become obsolete, as the general semantic has not changed, a simple conversion of the
  ...
  the policy.
- The existing contract definitions need to be adapted to a new access policy because with the migrated one, they will
  be filtered out during catalog creation. These contract definition will after the alteration still fulfill the
  purpose of defining data offers for Jupiter consumers
...
A recommended playground to expriment with the new policieis is the Policy Builder which will soon be an Eclipse
