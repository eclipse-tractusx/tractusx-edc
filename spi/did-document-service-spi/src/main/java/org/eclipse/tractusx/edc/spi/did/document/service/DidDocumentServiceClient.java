 ...
 * </pre>
 * <p>
 * The <code>service</code> list can be extended to advertise additional endpoints, such as DSP endpoints, data plane URLs, or other custom services.
 * <p>
 ...
 * <p>
 * Implementations should ensure that changes to the DID document are performed according to the underlying wallet or DID registry's requirements and that updates are properly propagated.
 */
@ExtensionPoint
public interface DidDocumentServiceClient {

    ServiceResult<Void> create(Service service);
    ...
    default ServiceResult<List<Service>> findAllByType(String type) {
        return findAll().map(services -> services.stream().filter(service -> service.getType().equals(type)).toList());
    }
}
